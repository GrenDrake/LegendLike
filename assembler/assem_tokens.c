...
static int convert_hex_digit(int digit) {
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        switch(text[i]) {
            ...
            case 'x': {
                int rpos = i + 1;
                ++escape_length;
                int value = convert_hex_digit(text[rpos]);
                if (value < 0) return rpos;
                ++rpos;
                int value2 = convert_hex_digit(text[rpos]);
                if (value2 >= 0) {
                    value *= 16;
                    ++escape_length;
                    value += value2;
    ...
    if (fp) {
        ...
        while (in != EOF) {
            if (in == '\n') {
                ...
                a_token = new_token(tt_at, NULL, &state);
                add_token(tokens, a_token);
                in = next_char(fp, &state);
